$().ready(function () {
     ...
     *       However it's currently slower than this judging from my tests.
    ...
    function handleFileSelect(evt) {
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            (function () {
                ...
                $("#list").empty().append('<li id="' + uid + '" class="entrystyle">' // Remove ".empty()" to allow multiple files list
                    + '<b>' + escapeHtml(f.name) + ' <span class="progresstext"></span></b>'
                    + '<div class="progress"></div>'
                    + '</li>');
					
					document.getElementById("filename").value = escapeHtml(f.name); //RomelSan
                ...
                function (file) {
					...
					document.getElementById("crc32").value = decimalToHexString(crc32intermediate); //RomelSan

                    for (var j = 0; j < enabledAlgorithms.length ; j++) {
						var algo_result = enabledAlgorithms[j].instance.finalize(); //RomelSan
                        results += '<tr><td class="algoname">' + enabledAlgorithms[j].name + ':</td><td class="algoresult">' + algo_result + '</td></tr>';// RomelSan
						if (enabledAlgorithms[j].name == "SHA256") {document.getElementById("sha256").value = algo_result;} // RomelSan
                    }
